...
import dev.lb.simplebase.util.function.BooleanToObjectFunction;
...
public final class PrimitiveTasks {
	 ...
	 */
	public static <V> TaskOf<V> chainInBool(TaskOfBool inner, BooleanToObjectFunction<? extends V> operation) {
		TaskCompleterOf<V> tco = TaskCompleterOf.create();
		TaskOf<V> resultTask = Tasks.startBlocking(tco);
		inner.onSuccess(value -> {
			try {
				tco.signalSuccess(operation.applyBoolean(value));
			} catch (Throwable e) {
	 ...
	 * @return A {@link Task} that waits for the inner task to finish and then applies a function to the result.
	 */
	public static <V> TaskOf<V> chainInAsyncBool(TaskOfBool inner, BooleanToObjectFunction<? extends V> operation) {
	 ...
	 * @return A {@link Task} that waits for the inner task to finish and then applies a function to the result.
	 */
	public static <V> TaskOf<V> chainInAsyncBool(TaskOfBool inner, BooleanToObjectFunction<? extends V> operation, ExecutorService executor) {
		TaskCompleterOf<V> tco = TaskCompleterOf.create();
		TaskOf<V> resultTask = Tasks.startBlocking(tco);
		inner.onSuccessAsync(value -> tco.signalSuccess(operation.applyBoolean(value)), executor);
